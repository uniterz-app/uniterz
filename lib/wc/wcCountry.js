...
exports.WC_COUNTRY_ISO3_LIST = void 0;
exports.teamIdToWcCountry = teamIdToWcCountry;
exports.teamIdToFlagClass = teamIdToFlagClass;
exports.teamIdToCountryName = teamIdToCountryName;
const WC_COUNTRIES = {
    // === Group A ===
 ...
 */
exports.WC_COUNTRY_ISO3_LIST = Object.keys(WC_COUNTRIES);
/**
 * teamId（"wc-jpn" 形式）を WcCountry に解決する。
 * 知らない国 / フォーマット外は null。
 */
function teamIdToWcCountry(teamId) {
    var _a;
    if (!teamId)
        return null;
    ...
    return (_a = WC_COUNTRIES[iso3]) !== null && _a !== void 0 ? _a : null;
}
/**
 * teamId から flag-icons の class 名を返す（"fi fi-jp" 形式）。
 * 解決できないときは null。
 */
function teamIdToFlagClass(teamId) {
 ...
 * 表示名（言語に応じて切替）
 */
function teamIdToCountryName(teamId, language) {
    const c = teamIdToWcCountry(teamId);
    if (!c)
        return null;
    return language === "ja" ? c.nameJa : c.name;
}
//# sourceMappingURL=wcCountry.js.map