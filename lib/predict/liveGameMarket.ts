/** 投稿作成・編集・削除時に games へ載せるライブ市場偏り */
...
/** marketPickCounts（ライブ）→ settle 後 market.*Count の順 */
...
/** 勝敗差し替え（編集） */
export function swapLiveMarketSide(
  counts: LiveMarketCounts,
  from: LiveMarketSide,
  to: LiveMarketSide
): LiveGameMarketPatch {
  if (from === to) return liveGameMarketFromCounts(counts);
  const afterDec = applyLiveMarketDelta(counts, from, -1);
  return applyLiveMarketDelta(afterDec.marketPickCounts, to, 1);
}
