...
import {
  KNICKS_CAVALIERS_TEAM_IDS,
  knicksCavaliersH2HAveragesForSides,
  knicksCavaliersH2HGames,
} from "./knicks-cavaliers-regular-2526";
import {
...
import {
  PISTONS_CAVALIERS_TEAM_IDS,
  pistonsCavaliersH2HAveragesForSides,
  pistonsCavaliersH2HGames,
} from "./pistons-cavaliers-regular-2526";
import {
  MIN_DEN_TEAM_IDS,
...
import {
  SPURS_KNICKS_TEAM_IDS,
  spursKnicksH2HAveragesForSides,
  spursKnicksH2HGames,
} from "./spurs-knicks-regular-2526";
import {
  SPURS_THUNDER_TEAM_IDS,
  spursThunderH2HAveragesForSides,
  spursThunderH2HGames,
} from "./spurs-thunder-regular-2526";
import {
...
function idsAreKnicksCavaliers(homeTeamId: string, awayTeamId: string): boolean {
  const s = new Set([homeTeamId, awayTeamId]);
  return KNICKS_CAVALIERS_TEAM_IDS.every((id) => s.has(id));
}

function idsArePistonsCavaliers(homeTeamId: string, awayTeamId: string): boolean {
  const s = new Set([homeTeamId, awayTeamId]);
  return PISTONS_CAVALIERS_TEAM_IDS.every((id) => s.has(id));
}
...
function idsAreSpursKnicks(homeTeamId: string, awayTeamId: string): boolean {
  const s = new Set([homeTeamId, awayTeamId]);
  return SPURS_KNICKS_TEAM_IDS.every((id) => s.has(id));
}

function idsAreSpursThunder(homeTeamId: string, awayTeamId: string): boolean {
  const s = new Set([homeTeamId, awayTeamId]);
  return SPURS_THUNDER_TEAM_IDS.every((id) => s.has(id));
}
...
function namesLookKnicksCavaliers(homeName?: string, awayName?: string): boolean {
  const blob = `${(homeName ?? "").toLowerCase()} ${(awayName ?? "").toLowerCase()}`;
  const hasKnicks = blob.includes("knick") || blob.includes("new york");
  const hasCavalier =
    blob.includes("cavalier") ||
    blob.includes("cleveland");
  const hasPiston = blob.includes("piston") || blob.includes("detroit");
  const hasHawk = blob.includes("hawk") || blob.includes("atlanta");
  const hasSixers =
    blob.includes("76er") ||
    blob.includes("sixer") ||
    blob.includes("philadelphia");
  const hasRaptor = blob.includes("raptor") || blob.includes("toronto");
  return hasKnicks && hasCavalier && !hasPiston && !hasHawk && !hasSixers && !hasRaptor;
}

function inferKnicksCavaliersIdsFromNames(
  homeName?: string,
  awayName?: string
): { homeTeamId: string; awayTeamId: string } | null {
  if (!namesLookKnicksCavaliers(homeName, awayName)) return null;
  const hn = (homeName ?? "").toLowerCase();
  const an = (awayName ?? "").toLowerCase();
  let hid = "";
  let aid = "";
  if (hn.includes("knick") || hn.includes("new york")) hid = "nba-knicks";
  else if (hn.includes("cavalier") || hn.includes("cleveland"))
    hid = "nba-cavaliers";
  if (an.includes("knick") || an.includes("new york")) aid = "nba-knicks";
  else if (an.includes("cavalier") || an.includes("cleveland"))
    aid = "nba-cavaliers";
  if (!hid || !aid || hid === aid) return null;
  return { homeTeamId: hid, awayTeamId: aid };
}

function namesLookPistonsCavaliers(homeName?: string, awayName?: string): boolean {
  const blob = `${(homeName ?? "").toLowerCase()} ${(awayName ?? "").toLowerCase()}`;
  const hasPiston = blob.includes("piston") || blob.includes("detroit");
  const hasCavalier =
    blob.includes("cavalier") ||
    blob.includes("cleveland");
  return hasPiston && hasCavalier;
}

function inferPistonsCavaliersIdsFromNames(
  homeName?: string,
  awayName?: string
): { homeTeamId: string; awayTeamId: string } | null {
  if (!namesLookPistonsCavaliers(homeName, awayName)) return null;
  const hn = (homeName ?? "").toLowerCase();
  const an = (awayName ?? "").toLowerCase();
  let hid = "";
  let aid = "";
...
function namesLookSpursKnicks(homeName?: string, awayName?: string): boolean {
  const blob = `${(homeName ?? "").toLowerCase()} ${(awayName ?? "").toLowerCase()}`;
  const hasSpurs = blob.includes("spur") || blob.includes("san antonio");
  const hasKnicks = blob.includes("knick") || blob.includes("new york");
  return hasSpurs && hasKnicks;
}

function inferSpursKnicksIdsFromNames(
  homeName?: string,
  awayName?: string
): { homeTeamId: string; awayTeamId: string } | null {
  if (!namesLookSpursKnicks(homeName, awayName)) return null;
  const hn = (homeName ?? "").toLowerCase();
  const an = (awayName ?? "").toLowerCase();
  let hid = "";
  let aid = "";
  if (hn.includes("spur") || hn.includes("san antonio")) hid = "nba-spurs";
  else if (hn.includes("knick") || hn.includes("new york")) hid = "nba-knicks";
  if (an.includes("spur") || an.includes("san antonio")) aid = "nba-spurs";
  else if (an.includes("knick") || an.includes("new york")) aid = "nba-knicks";
  if (!hid || !aid || hid === aid) return null;
  return { homeTeamId: hid, awayTeamId: aid };
}
...
function namesLookSpursThunder(homeName?: string, awayName?: string): boolean {
  const blob = `${(homeName ?? "").toLowerCase()} ${(awayName ?? "").toLowerCase()}`;
  const hasSpurs =
    blob.includes("spur") ||
    blob.includes("san antonio");
  const hasThunder =
    ...
    blob.includes("oklahoma") ||
    /\bokc\b/.test(blob);
  return hasSpurs && hasThunder;
}

function inferSpursThunderIdsFromNames(
  homeName?: string,
  awayName?: string
): { homeTeamId: string; awayTeamId: string } | null {
  if (!namesLookSpursThunder(homeName, awayName)) return null;
  const hn = (homeName ?? "").toLowerCase();
  const an = (awayName ?? "").toLowerCase();
  let hid = "";
  let aid = "";
...
export function resolveNbaH2HPack(
  ...
  if (!hid || !aid) {
    ...
    const inferredKnicksCavaliers = inferKnicksCavaliersIdsFromNames(
      homeName,
      awayName
    );
    const inferredPistonsCavaliers = inferPistonsCavaliersIdsFromNames(
      homeName,
      awayName
    );
    ...
    const inferredSpursKnicks = inferSpursKnicksIdsFromNames(
      homeName,
      awayName
    );
    ...
    const inferredSpursThunder = inferSpursThunderIdsFromNames(
      homeName,
      awayName
    );
    const inferred =
      ...
      inferredKnicksCavaliers ??
      inferredPistonsCavaliers ??
  ...
  if (idsAreKnicksCavaliers(hid, aid)) {
    const h2hAverages = knicksCavaliersH2HAveragesForSides({
      homeTeamId: hid,
      awayTeamId: aid,
    });
    if (!h2hAverages) return null;
    return {
      games: knicksCavaliersH2HGames,
      h2hAverages,
      seriesRecord: computeH2hSeriesRecord(knicksCavaliersH2HGames),
    };
  }

  if (idsArePistonsCavaliers(hid, aid)) {
    const h2hAverages = pistonsCavaliersH2HAveragesForSides({
      homeTeamId: hid,
      awayTeamId: aid,
    });
    if (!h2hAverages) return null;
    return {
      games: pistonsCavaliersH2HGames,
      h2hAverages,
      seriesRecord: computeH2hSeriesRecord(pistonsCavaliersH2HGames),
    };
  }
  ...
  if (idsAreSpursKnicks(hid, aid)) {
    const h2hAverages = spursKnicksH2HAveragesForSides({
      homeTeamId: hid,
      awayTeamId: aid,
    });
    if (!h2hAverages) return null;
    return {
      games: spursKnicksH2HGames,
      h2hAverages,
      seriesRecord: computeH2hSeriesRecord(spursKnicksH2HGames),
    };
  }

  if (idsAreSpursThunder(hid, aid)) {
    const h2hAverages = spursThunderH2HAveragesForSides({
      homeTeamId: hid,
      awayTeamId: aid,
    });
    if (!h2hAverages) return null;
    return {
      games: spursThunderH2HGames,
      h2hAverages,
      seriesRecord: computeH2hSeriesRecord(spursThunderH2HGames),
    };
  }
