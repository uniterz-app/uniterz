/**
 * wc-2026-C-bra-hti（ブラジル 3–0 ハイチ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   23' マテウス・クーニャ — bra-cunha
 *   36' マテウス・クーニャ — bra-cunha
 *   45+3' ヴィニシウス・ジュニオール — bra-junior（保存値 48）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-bra-hti.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-bra-hti.ts
 *   npx tsx scripts/set-wc-goal-scorers-bra-hti.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-bra-hti.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-bra-hti.ts --force
 *
 * 汎用 CLI:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-C-bra-hti --file=scripts/data/wc-goal-scorers-bra-hti.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（3–0 確定）
...
const DEFAULT_GAME_ID = "wc-2026-C-bra-hti";

/** ブラジル 3–0 ハイチ（23' 36' クーニャ / 45+3' ヴィニシウス） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "bra-cunha", teamId: "wc-bra", minute: 23 },
  { playerId: "bra-cunha", teamId: "wc-bra", minute: 36 },
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 0, final: true");
  }
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-bra-hti.ts --with-score --resettle --force"
    );
  }
