/**
 * wc-2026-ko-M91（ブラジル 1–2 ノルウェー / ベスト16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   79' アーリング・ブラウト・ハーランド（ノルウェー）— nor-haaland
 *   90' アーリング・ブラウト・ハーランド（ノルウェー）— nor-haaland
 *   90+10' ネイマール（ブラジル / PK）— bra-neymar
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M91-bra-nor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M91-bra-nor.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M91-bra-nor.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M91-bra-nor.ts --with-score --resettle --force
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M91 --file=scripts/data/wc-goal-scorers-ko-M91-bra-nor.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（1–2 確定）
...
const DEFAULT_GAME_ID = "wc-2026-ko-M91";

/** ブラジル（ホーム）1 – 2 ノルウェー（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nor-haaland", teamId: "wc-nor", minute: 79 },
  { playerId: "nor-haaland", teamId: "wc-nor", minute: 90 },
  { playerId: "bra-neymar", teamId: "wc-bra", minute: 100 },
];
