 ...
 *
 *   npx tsx scripts/ingest-nba-player-game-logs-from-bdl.ts
 *   npx tsx scripts/ingest-nba-player-game-logs-from-bdl.ts 2026-27
 *   npx tsx scripts/ingest-nba-player-game-logs-from-bdl.ts 2026-27 175
 *   npx tsx scripts/ingest-nba-player-game-logs-from-bdl.ts 2025-26 175
 *
...
async function main() {
  ...
  const playerIds = playerIdsRaw
    ...
    : undefined;
  const maxPlayersRaw = (process.argv[4] ?? "").trim();
  const maxPlayers = maxPlayersRaw
    ? Number.parseInt(maxPlayersRaw, 10)
    : undefined;
  const result = await ingestNbaPlayerGameLogsFromBdl(getAdminDb(), {
    seasonKey,
    playerIds,
    maxPlayers:
      maxPlayers != null && Number.isFinite(maxPlayers) && maxPlayers > 0
        ? maxPlayers
        : undefined,
  });
