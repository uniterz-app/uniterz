/**
 * wc-2026-C-hti-sct（ハイチ 0–1 スコットランド）の goalScorers を Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-hti-sct.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-hti-sct.ts
 *   npx tsx scripts/set-wc-goal-scorers-hti-sct.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-hti-sct.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-hti-sct.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-C-hti-sct";

/** スコットランド 28' — John McGinn */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "sct-mcginn", teamId: "wc-sct", minute: 28 },
];
