/**
 * wc-2026-ko-M81（アメリカ 2–0 ボスニア・ヘルツェゴビナ / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   45' フォラリン・バログン（アメリカ）— usa-balogun
 *   82' マリク・ティルマン（アメリカ）— usa-tillman
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M81-usa-bih.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M81-usa-bih.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M81-usa-bih.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M81-usa-bih.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M81";

/** アメリカ（ホーム）2 – 0 ボスニア・ヘルツェゴビナ（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "usa-balogun", teamId: "wc-usa", minute: 45 },
  { playerId: "usa-tillman", teamId: "wc-usa", minute: 82 },
];
