/**
 * wc-2026-J-dza-aut（アルジェリア 3–3 オーストリア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   28' マルコ・アルナウトヴィッチ（オーストリア）
 *   45' ラフィク・ベルガリ（アルジェリア）
 *   55' マルセル・ザビッツァー（オーストリア）
 *   60' リヤド・マフレズ（アルジェリア）
 *   90+3' リヤド・マフレズ（アルジェリア）
 *   90+6' サーシャ・カライジッチ（オーストリア）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-dza-aut.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-dza-aut.ts
 *   npx tsx scripts/set-wc-goal-scorers-dza-aut.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-dza-aut.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-dza-aut.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（3–3 確定）
...
const DEFAULT_GAME_ID = "wc-2026-J-dza-aut";
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 3, final: true");
  }
