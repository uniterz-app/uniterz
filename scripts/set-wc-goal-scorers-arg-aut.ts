/**
 * wc-2026-J-arg-aut（アルゼンチン 2–0 オーストリア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   38'  リオネル・メッシ（アルゼンチン）
 *   90+5' リオネル・メッシ（アルゼンチン）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-arg-aut.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-arg-aut.ts
 *   npx tsx scripts/set-wc-goal-scorers-arg-aut.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-arg-aut.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-arg-aut.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-J-arg-aut";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "arg-messi", teamId: "wc-arg", minute: 38 },
  { playerId: "arg-messi", teamId: "wc-arg", minute: 95 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-arg-aut.ts --with-score --resettle --force"
    );
  }
