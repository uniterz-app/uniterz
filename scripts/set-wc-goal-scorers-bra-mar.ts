/**
 * wc-2026-C-bra-mar（ブラジル 1–1 モロッコ）の goalScorers を Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-bra-mar.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-bra-mar.ts
 *   npx tsx scripts/set-wc-goal-scorers-bra-mar.ts --force
 *   npx tsx scripts/set-wc-goal-scorers-bra-mar.ts --resettle
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-C-bra-mar";

/** モロッコ 21' → ブラジル 32' */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mar-saibari", teamId: "wc-mar", minute: 21 },
  { playerId: "bra-junior", teamId: "wc-bra", minute: 32 },
];
...
(async () => {
  ...
  const shouldResettle = RESETTLE && Boolean(data.final);
  ...
  } else if (RESETTLE && !data.final) {
    console.log(
      "\n⚠ --resettle は final 試合向けです。先にスコア確定を設定してください。"
    );
  }

  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log("  npx tsx scripts/set-wc-goal-scorers-bra-mar.ts");
    console.log("  npx tsx scripts/set-wc-goal-scorers-bra-mar.ts --resettle  # final 後");
  }
