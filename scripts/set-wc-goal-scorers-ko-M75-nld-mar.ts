/**
 * wc-2026-ko-M75（オランダ 1–1 モロッコ / PK 2–3 / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点（規定・延長のみ）:
 *   72' コーディ・ガクポ（オランダ）— nld-gakpo
 *   90+1' イッサ・ディオプ（モロッコ）— mar-diop
 *
 * PK 勝者: モロッコ（advancingTeamId = away）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M75-nld-mar.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M75-nld-mar.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M75-nld-mar.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M75-nld-mar.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M75";

/** オランダ（ホーム）1 – 1 モロッコ（アウェイ）→ PK 2–3 でモロッコ勝利 */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nld-gakpo", teamId: "wc-nld", minute: 72 },
  { playerId: "mar-diop", teamId: "wc-mar", minute: 91 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 1 (PK 2-3), final: true");
