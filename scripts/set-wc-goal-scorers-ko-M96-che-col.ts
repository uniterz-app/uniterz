/**
 * wc-2026-ko-M96（スイス 0–0 コロンビア / PK 4–3 / ラウンド16）の goalScorers を Firestore に投入。
 *
 * 得点（規定・延長）: なし
 * PK 勝者: スイス（advancingTeamId = home / wc-che）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M96-che-col.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M96-che-col.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M96-che-col.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M96-che-col.ts --with-score --resettle --force
 *   npx tsx scripts/set-wc-goal-scorers-ko-M96-che-col.ts --with-score --resettle --force --bracket-chain
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M96 --file=scripts/data/wc-goal-scorers-ko-M96-che-col.json --with-score --advancing-team-id=wc-che
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M96";

/** スイス（ホーム）0 – 0 コロンビア（アウェイ）→ PK 4–3 でスイス勝利 */
...
(async () => {
  ...
  if (WITH_SCORE) {
    ...
    patch.regulationEtScore = { home: 0, away: 0 };
    patch.advancingTeamId = homeTeamId;
    patch.pkScore = { home: 4, away: 3 };
    patch.final = true;
    patch.status = "final";
  ...
  if (WITH_SCORE) {
    console.log("score: 0 - 0 (PK 4-3), final: true");
    console.log("advancingTeamId:", homeTeamId);
  }
