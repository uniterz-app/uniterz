/**
 * wc-2026-ko-M83（ポルトガル 2–1 クロアチア・R32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   53' イヴァン・ペリシッチ（クロアチア）
 *   68' PK クリスティアーノ・ロナウド（ポルトガル）
 *   90+4' ゴンサロ・ラモス（ポルトガル）
 *
 *   npx tsx scripts/set-wc-goal-scorers-prt-hrv.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-prt-hrv.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M83";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "hrv-perisic", teamId: "wc-hrv", minute: 53 },
  { playerId: "prt-ronaldo", teamId: "wc-prt", minute: 68 },
  { playerId: "prt-ramos", teamId: "wc-prt", minute: 94 },
];
...
(async () => {
  ...
  if (WITH_SCORE) console.log("score: 2 - 1, final: true");
