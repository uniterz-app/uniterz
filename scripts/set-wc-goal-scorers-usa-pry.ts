/**
 * wc-2026-D-usa-pry（USA 4–1 Paraguay）の goalScorers を ID 形式で Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-usa-pry.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-usa-pry.ts
 *   npx tsx scripts/set-wc-goal-scorers-usa-pry.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-usa-pry.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-usa-pry.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-D-usa-pry";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "pry-bobadilla", teamId: "wc-pry", minute: 7, ownGoal: true },
  { playerId: "usa-balogun", teamId: "wc-usa", minute: 31 },
  { playerId: "usa-balogun", teamId: "wc-usa", minute: 50 },
  { playerId: "pry-mauricio", teamId: "wc-pry", minute: 73 },
  { playerId: "usa-reyna", teamId: "wc-usa", minute: 98 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log("  npx tsx scripts/set-wc-goal-scorers-usa-pry.ts --with-score --resettle");
  }
