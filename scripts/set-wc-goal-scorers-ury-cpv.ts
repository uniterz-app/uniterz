/**
 * wc-2026-H-ury-cpv（ウルグアイ 2–2 カーボベルデ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   21' ケビン・ピナ（カーボベルデ）
 *   44' マクシミリアーノ・アラウホ（ウルグアイ）
 *   45+6' アグスティン・カノッビオ（ウルグアイ）
 *   61' エリオ・ヴァレーラ（カーボベルデ）
 *
 *   npx tsx scripts/set-wc-goal-scorers-ury-cpv.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ury-cpv.ts --with-score --kickoff-today-jst
 *   npx tsx scripts/set-wc-goal-scorers-ury-cpv.ts --with-score --kickoff-today-jst --force --resettle
 */
...
const DEFAULT_GAME_ID = "wc-2026-H-ury-cpv";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "cpv-pina", teamId: "wc-cpv", minute: 21 },
  { playerId: "ury-araujo-2", teamId: "wc-ury", minute: 44 },
  { playerId: "ury-canobbio", teamId: "wc-ury", minute: 51 },
  { playerId: "cpv-varela", teamId: "wc-cpv", minute: 61 },
];
...
function todayAt18JstDate(): Date {
  const now = new Date();
  const jst = new Date(now.getTime() + 9 * 60 * 60 * 1000);
  const y = jst.getUTCFullYear();
  const m = String(jst.getUTCMonth() + 1).padStart(2, "0");
  const d = String(jst.getUTCDate()).padStart(2, "0");
  return new Date(`${y}-${m}-${d}T18:00:00+09:00`);
}

(async () => {
  console.log(`=== set ury-cpv: ${GAME_ID} ===`);
  ...
  console.error("set ury-cpv failed:", e);
  process.exit(1);
});
