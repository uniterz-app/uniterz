/**
 * wc-2026-ko-M82（ベルギー 3–2 セネガル・R32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   24' アビブ・ディアッラ（セネガル）
 *   51' イスマイラ・サール（セネガル）
 *   86' ロメル・ルカク（ベルギー）
 *   89' ユーリ・ティーレマンス（ベルギー）
 *  125' ユーリ・ティーレマンス PK（120+5'・ベルギー）
 *
 *   npx tsx scripts/set-wc-goal-scorers-bel-sen.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-bel-sen.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M82";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "sen-diarra", teamId: "wc-sen", minute: 24 },
  { playerId: "sen-sarr-3", teamId: "wc-sen", minute: 51 },
  { playerId: "bel-lukaku", teamId: "wc-bel", minute: 86 },
  { playerId: "bel-tielemans", teamId: "wc-bel", minute: 89 },
  { playerId: "bel-tielemans", teamId: "wc-bel", minute: 125 },
];
...
(async () => {
  ...
  if (WITH_SCORE) console.log("score: 3 - 2, final: true");
