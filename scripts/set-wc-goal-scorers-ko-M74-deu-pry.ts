/**
 * wc-2026-ko-M74（ドイツ 1–1 パラグアイ / PK 3–4 / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点（規定・延長のみ）:
 *   42' フリオ・エンシソ（パラグアイ）— pry-enciso
 *   54' カイ・ハフェルツ（ドイツ）— deu-havertz
 *
 * PK 勝者: パラグアイ（advancingTeamId = away）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M74-deu-pry.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M74-deu-pry.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M74-deu-pry.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M74-deu-pry.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M74";

/** ドイツ（ホーム）1 – 1 パラグアイ（アウェイ）→ PK 3–4 でパラグアイ勝利 */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "pry-enciso", teamId: "wc-pry", minute: 42 },
  { playerId: "deu-havertz", teamId: "wc-deu", minute: 54 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    patch.homeScore = 1;
    patch.awayScore = 1;
    patch.score = { home: 1, away: 1 };
    patch.regulationEtScore = { home: 1, away: 1 };
    patch.advancingTeamId = awayTeamId;
    patch.pkScore = { home: 3, away: 4 };
    patch.final = true;
    patch.status = "final";
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 1 (PK 3-4), final: true");
    console.log("advancingTeamId:", awayTeamId);
  }
