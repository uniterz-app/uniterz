/**
 * PO カンファレンスファイナル用バッジを1ユーザーに5つまとめて付与（表示・サイズ確認用）
 *
 *   npx tsx scripts/grant-po-2026-cf-badges-test.ts
 *
 * 別 UID:
 *   TEST_UID=xxxx npx tsx scripts/grant-po-2026-cf-badges-test.ts
 *
 * 前提: scripts/seed-po-2026-cf-badges.ts を実行済み（master_badges に icon あり）
 */
...
async function main() {
  console.log(`=== grant PO CF badges (test) -> ${TEST_UID} ===`);

  for (const badgeId of BADGE_IDS) {
    const master = await db.collection("master_badges").doc(badgeId).get();
    if (!master.exists) {
      console.warn(
        `master_badges/${badgeId} がありません。先に npx tsx scripts/seed-po-2026-cf-badges.ts を実行してください。`
      );
    } else {
      const d = master.data();
