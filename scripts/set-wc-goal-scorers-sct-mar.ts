/**
 * wc-2026-C-sct-mar（スコットランド 0–1 モロッコ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   2' イスマエル・サイバリ — mar-saibari
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-sct-mar.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-sct-mar.ts
 *   npx tsx scripts/set-wc-goal-scorers-sct-mar.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-sct-mar.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-sct-mar.ts --force
 *
 * 汎用 CLI:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-C-sct-mar --file=scripts/data/wc-goal-scorers-sct-mar.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-C-sct-mar";

/** スコットランド 0–1 モロッコ（2' サイバリ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mar-saibari", teamId: "wc-mar", minute: 2 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-sct-mar.ts --with-score --resettle --force"
    );
  }
