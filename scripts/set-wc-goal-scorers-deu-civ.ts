/**
 * wc-2026-E-deu-civ（ドイツ 2–1 コートジボワール）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   30'  フランク・ケシエ（コートジボワール）
 *   68'  デニズ・ウンダフ（ドイツ）
 *   90+4' デニズ・ウンダフ（ドイツ）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-deu-civ.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-deu-civ.ts
 *   npx tsx scripts/set-wc-goal-scorers-deu-civ.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-deu-civ.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-deu-civ.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（2–1 確定）
...
const DEFAULT_GAME_ID = "wc-2026-E-deu-civ";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "civ-kessie", teamId: "wc-civ", minute: 30 },
  { playerId: "deu-undav", teamId: "wc-deu", minute: 68 },
  { playerId: "deu-undav", teamId: "wc-deu", minute: 94 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-deu-civ.ts --with-score --resettle --force"
    );
  }
