 ...
 *
 *   npx tsx scripts/ingest-nba-player-shot-zones-from-bdl.ts
 *   npx tsx scripts/ingest-nba-player-shot-zones-from-bdl.ts 2026-27
 *   npx tsx scripts/ingest-nba-player-shot-zones-from-bdl.ts 2026-27 175
 *
...
async function main() {
  ...
  const maxPlayers = maxPlayersRaw
    ...
    : undefined;
  const result = await ingestNbaPlayerShotZonesFromBdl(getAdminDb(), {
    seasonKey,
    playerIds,
    maxPlayers:
