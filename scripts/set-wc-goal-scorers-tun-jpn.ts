/**
 * wc-2026-F-tun-jpn（チュニジア 0–4 日本）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   4'  鎌田大地（日本）
 *   31' 上田綺世（日本）
 ...
 *
 *   npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts
 *   npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（0–4 確定）
...
const DEFAULT_GAME_ID = "wc-2026-F-tun-jpn";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "jpn-kamada", teamId: "wc-jpn", minute: 4 },
  { playerId: "jpn-ueda", teamId: "wc-jpn", minute: 31 },
  { playerId: "jpn-ito", teamId: "wc-jpn", minute: 69 },
  { playerId: "jpn-ueda", teamId: "wc-jpn", minute: 83 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 0 - 4, final: true");
  }
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-tun-jpn.ts --with-score --resettle --force"
    );
  }
