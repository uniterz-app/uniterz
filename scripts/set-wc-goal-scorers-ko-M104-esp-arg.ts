/**
 * wc-2026-ko-M104（スペイン 1–0 アルゼンチン / 決勝）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   106' フェラン・トーレス（スペイン）— esp-torres（延長前半）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M104-esp-arg.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M104-esp-arg.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M104-esp-arg.ts --with-score --bracket-chain
 *   npx tsx scripts/set-wc-goal-scorers-ko-M104-esp-arg.ts --with-score --resettle --force
 *
 * 注意: 試合が既に final=true の状態で投入すると onGameFinalV2（投稿精算）が走りません。
 *       投稿が未精算のときは scripts/retrigger-on-game-final-v2.ts を実行してください。
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M104 --file=scripts/data/wc-goal-scorers-ko-M104-esp-arg.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M104";

/** スペイン（ホーム）1 – 0 アルゼンチン（アウェイ）。延長 106' トーレス */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "esp-torres", teamId: "wc-esp", minute: 106 },
];
...
(async () => {
  ...
  /** 決勝: home=準決勝M101勝者(スペイン) / away=準決勝M102勝者(アルゼンチン) */
  const EXPECTED_HOME = "wc-esp";
