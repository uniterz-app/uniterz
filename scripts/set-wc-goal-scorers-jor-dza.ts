/**
 * wc-2026-J-jor-dza（ヨルダン 1–2 アルジェリア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   36' ニザール・アル＝ラシュダン（ヨルダン）
 *   69' ナディール・ベンブアリ（アルジェリア）
 *   82' アミーヌ・グイリ（アルジェリア）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-jor-dza.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-jor-dza.ts
 *   npx tsx scripts/set-wc-goal-scorers-jor-dza.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-jor-dza.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-jor-dza.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-J-jor-dza";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "jor-al-rashdan", teamId: "wc-jor", minute: 36 },
  { playerId: "dza-benbouali", teamId: "wc-dza", minute: 69 },
  { playerId: "dza-gouiri", teamId: "wc-dza", minute: 82 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-jor-dza.ts --with-score --resettle --force"
    );
  }
