/**
 * wc-2026-I-fra-sen（フランス 3–1 セネガル）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   66' キリアン・エムバペ（フランス）
 *   82' ブラッドリー・バルコラ（フランス）
 *   90+5' イブラヒム・エンバイェ（セネガル）
 *   90+6' キリアン・エムバペ（フランス）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-fra-sen.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-fra-sen.ts
 *   npx tsx scripts/set-wc-goal-scorers-fra-sen.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-fra-sen.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-fra-sen.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-I-fra-sen";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 66 },
  { playerId: "fra-barcola", teamId: "wc-fra", minute: 82 },
  { playerId: "sen-mbaye", teamId: "wc-sen", minute: 95 },
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 96 },
];
