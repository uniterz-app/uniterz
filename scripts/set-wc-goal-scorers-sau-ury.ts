/**
 * wc-2026-H-sau-ury（サウジアラビア 1–1 ウルグアイ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   41' アブドゥレラー・アル＝アムリ（サウジアラビア）
 *   80' マクシミリアーノ・アラウホ（ウルグアイ）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-sau-ury.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-sau-ury.ts
 *   npx tsx scripts/set-wc-goal-scorers-sau-ury.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-sau-ury.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-sau-ury.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-H-sau-ury";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "sau-al-amri", teamId: "wc-sau", minute: 41 },
  { playerId: "ury-araujo-2", teamId: "wc-ury", minute: 80 },
];
