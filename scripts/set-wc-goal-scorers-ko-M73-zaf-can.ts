/**
 * wc-2026-ko-M73（南アフリカ 0–1 カナダ / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   90+2' ステファン・エウスタキオ（カナダ）— can-eustaquio
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M73-zaf-can.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M73-zaf-can.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M73-zaf-can.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M73-zaf-can.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M73";

/** 南アフリカ（ホーム）0 – 1 カナダ（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "can-eustaquio", teamId: "wc-can", minute: 92 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    ...
    patch.regulationEtScore = { home: 0, away: 1 };
