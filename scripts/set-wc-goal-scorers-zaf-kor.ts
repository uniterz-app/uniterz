/**
 * wc-2026-A-zaf-kor（南アフリカ 1–0 韓国）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   63'  タペロ・マセコ（南アフリカ）— zaf-maseko
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-zaf-kor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-zaf-kor.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-zaf-kor.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-zaf-kor.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-A-zaf-kor";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "zaf-maseko", teamId: "wc-zaf", minute: 63 },
];
