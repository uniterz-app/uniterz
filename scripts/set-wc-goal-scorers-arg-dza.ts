/**
 * wc-2026-J-arg-dza（アルゼンチン 3–0 アルジェリア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   17' リオネル・メッシ（アルゼンチン）
 *   60' リオネル・メッシ（アルゼンチン）
 ...
 *
 *   npx tsx scripts/set-wc-goal-scorers-arg-dza.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-arg-dza.ts
 *   npx tsx scripts/set-wc-goal-scorers-arg-dza.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-arg-dza.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-arg-dza.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-J-arg-dza";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "arg-messi", teamId: "wc-arg", minute: 17 },
  { playerId: "arg-messi", teamId: "wc-arg", minute: 60 },
  { playerId: "arg-messi", teamId: "wc-arg", minute: 76 },
];
