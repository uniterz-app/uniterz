/**
 * wc-2026-F-swe-tun（スウェーデン 5–1 チュニジア）の goalScorers を Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-swe-tun.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-swe-tun.ts
 *   npx tsx scripts/set-wc-goal-scorers-swe-tun.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-swe-tun.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-swe-tun.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-F-swe-tun";

/** スウェーデン 7', 30', 59', 84', 90+6' → チュニジア 43' */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "swe-ayari", teamId: "wc-swe", minute: 7 },
  { playerId: "swe-isak", teamId: "wc-swe", minute: 30 },
  { playerId: "tun-rekik", teamId: "wc-tun", minute: 43 },
  { playerId: "swe-gyokeres", teamId: "wc-swe", minute: 59 },
  { playerId: "swe-svanberg", teamId: "wc-swe", minute: 84 },
  { playerId: "swe-ayari", teamId: "wc-swe", minute: 96 },
];
