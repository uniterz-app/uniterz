/**
 * wc-2026-K-col-cod（コロンビア 1–0 DRコンゴ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   76' ダニエル・ムニョス（コロンビア）
 *
 ...
 *
 *   npx tsx scripts/set-wc-goal-scorers-col-cod.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-col-cod.ts
 *   npx tsx scripts/set-wc-goal-scorers-col-cod.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-col-cod.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-col-cod.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（1–0 確定）
...
const DEFAULT_GAME_ID = "wc-2026-K-col-cod";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "col-munoz", teamId: "wc-col", minute: 76 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 0, final: true");
  }
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-col-cod.ts --with-score --resettle --force"
    );
  }
