/**
 * wc-2026-G-bel-egy（ベルギー 1–1 エジプト）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   20' エマム・アシュール（エジプト）
 *   66' ムハンマド・ハニー OG（ベルギー側表示）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-bel-egy.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-bel-egy.ts
 *   npx tsx scripts/set-wc-goal-scorers-bel-egy.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-bel-egy.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-bel-egy.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-G-bel-egy";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "egy-ashour", teamId: "wc-egy", minute: 20 },
  { playerId: "egy-hany", teamId: "wc-egy", minute: 66, ownGoal: true },
];
