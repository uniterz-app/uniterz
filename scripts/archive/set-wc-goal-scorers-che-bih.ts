/**
 * wc-2026-B-che-bih（スイス 4–1 ボスニア・ヘルツェゴビナ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   74'  ヨハン・マンザンビ（スイス）— che-manzambi
 *   84'  ルベン・バルガス（スイス）— che-vargas
 *   90'  ヨハン・マンザンビ（スイス）— che-manzambi
 *   90+3' エルミン・マフミッチ（ボスニア）— bih-mahmic
 *   90+7' グラニト・ジャカ PK（スイス）— che-xhaka
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-che-bih.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-che-bih.ts --with-score --force
 *   npx tsx scripts/set-wc-goal-scorers-che-bih.ts --with-score --resettle --force
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-B-che-bih --file=scripts/data/wc-goal-scorers-che-bih.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（4–1 確定）
...
const DEFAULT_GAME_ID = "wc-2026-B-che-bih";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "che-manzambi", teamId: "wc-che", minute: 74 },
  { playerId: "che-vargas", teamId: "wc-che", minute: 84 },
  { playerId: "che-manzambi", teamId: "wc-che", minute: 90 },
  { playerId: "bih-mahmic", teamId: "wc-bih", minute: 93 },
  { playerId: "che-xhaka", teamId: "wc-che", minute: 97 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 4 - 1, final: true");
  }
