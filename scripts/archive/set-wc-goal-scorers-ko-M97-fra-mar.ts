/**
 * wc-2026-ko-M97（フランス 2–0 モロッコ / 準々決勝）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   60' キリアン・エムバペ（フランス）— fra-mbappe
 *   66' ウスマン・デンベレ（フランス）— fra-dembele
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M97-fra-mar.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M97-fra-mar.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M97-fra-mar.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M97-fra-mar.ts --with-score --resettle --force
 *
 * 注意: 試合が既に final=true の状態で投入すると onGameFinalV2（投稿精算）が走りません。
 *       投稿が未精算のときは scripts/retrigger-on-game-final-v2.ts を実行してください。
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M97 --file=scripts/data/wc-goal-scorers-ko-M97-fra-mar.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M97";

/** フランス（ホーム）2 – 0 モロッコ（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 60 },
  { playerId: "fra-dembele", teamId: "wc-fra", minute: 66 },
];
