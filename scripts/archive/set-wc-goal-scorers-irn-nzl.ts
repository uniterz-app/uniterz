/**
 * wc-2026-G-irn-nzl（イラン 2–2 ニュージーランド）の goalScorers を Firestore に投入。
 *
 * 得点:
 *    7' イライジャー・ジャスト（ニュージーランド）
 *   32' ラミン・レザイーアン（イラン）
 *   55' イライジャー・ジャスト（ニュージーランド）
 *   64' モハマド・モヘビ（イラン）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-irn-nzl.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-irn-nzl.ts
 *   npx tsx scripts/set-wc-goal-scorers-irn-nzl.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-irn-nzl.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-irn-nzl.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（2–2 確定）
...
const DEFAULT_GAME_ID = "wc-2026-G-irn-nzl";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nzl-just", teamId: "wc-nzl", minute: 7 },
  { playerId: "irn-rezaeian", teamId: "wc-irn", minute: 32 },
  { playerId: "nzl-just", teamId: "wc-nzl", minute: 55 },
  { playerId: "irn-mohebi", teamId: "wc-irn", minute: 64 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 2 - 2, final: true");
  }
