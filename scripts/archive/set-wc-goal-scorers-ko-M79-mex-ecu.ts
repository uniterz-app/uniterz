/**
 * wc-2026-ko-M79（メキシコ 2–0 エクアドル / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   22' フリアン・キニョネス（メキシコ）— mex-quinones
 *   31' ラウル・ヒメネス（メキシコ）— mex-jimenez
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M79-mex-ecu.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M79-mex-ecu.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M79-mex-ecu.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M79-mex-ecu.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M79";

/** メキシコ（ホーム）2 – 0 エクアドル（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mex-quinones", teamId: "wc-mex", minute: 22 },
  { playerId: "mex-jimenez", teamId: "wc-mex", minute: 31 },
];
