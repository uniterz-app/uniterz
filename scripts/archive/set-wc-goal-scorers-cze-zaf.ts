/**
 * wc-2026-A-cze-zaf（チェコ 1–1 南アフリカ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   6'  ミハル・サディレク（チェコ）— cze-sadilek
 *   83' テボホ・モコエナ PK（南アフリカ）— zaf-mokoena
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-cze-zaf.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-cze-zaf.ts
 *   npx tsx scripts/set-wc-goal-scorers-cze-zaf.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-cze-zaf.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-cze-zaf.ts --force
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-A-cze-zaf --file=scripts/data/wc-goal-scorers-cze-zaf.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（1–1 確定）
...
const DEFAULT_GAME_ID = "wc-2026-A-cze-zaf";

/** チェコ 6' — Michal Sadílek / 南アフリカ 83' — Teboho Mokoena (PK) */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "cze-sadilek", teamId: "wc-cze", minute: 6 },
  { playerId: "zaf-mokoena", teamId: "wc-zaf", minute: 83 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 1, final: true");
  }
