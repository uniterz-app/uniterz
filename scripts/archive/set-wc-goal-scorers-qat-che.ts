/**
 * wc-2026-B-qat-che（カタール 1–1 スイス）の goalScorers を Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-qat-che.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-qat-che.ts
 *   npx tsx scripts/set-wc-goal-scorers-qat-che.ts --force
 *   npx tsx scripts/set-wc-goal-scorers-qat-che.ts --resettle
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-B-qat-che";

/** スイス 17'（PK）→ カタール 90+5' */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "che-embolo", teamId: "wc-che", minute: 17 },
  { playerId: "qat-khoukhi", teamId: "wc-qat", minute: 95 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log("  npx tsx scripts/set-wc-goal-scorers-qat-che.ts");
    console.log("  npx tsx scripts/set-wc-goal-scorers-qat-che.ts --resettle  # final 後");
  }
