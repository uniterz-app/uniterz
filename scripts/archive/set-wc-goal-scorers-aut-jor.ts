/**
 * wc-2026-J-aut-jor（オーストリア 3–1 ヨルダン）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   21' ロマーノ・シュミット（オーストリア）
 *   50' アリー・オルワン（ヨルダン）
 *   76' ヤザン・アル＝アラブ OG（ヨルダン）
 *   90+12' マルコ・アルナウトヴィッチ PK（オーストリア）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-aut-jor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-aut-jor.ts
 *   npx tsx scripts/set-wc-goal-scorers-aut-jor.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-aut-jor.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-aut-jor.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（3–1 確定）
...
const DEFAULT_GAME_ID = "wc-2026-J-aut-jor";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "aut-schmid", teamId: "wc-aut", minute: 21 },
  { playerId: "jor-olwan", teamId: "wc-jor", minute: 50 },
  { playerId: "jor-al-arab", teamId: "wc-jor", minute: 76, ownGoal: true },
  { playerId: "aut-arnautovic", teamId: "wc-aut", minute: 102 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 1, final: true");
  }
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-aut-jor.ts --with-score --resettle --force"
    );
  }
