/**
 * wc-2026-K-uzb-col（ウズベキスタン 1–3 コロンビア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   40' ダニエル・ムニョス（コロンビア）
 *   60' アボスベク・ファイズラエフ（ウズベキスタン）
 *   65' ルイス・ディアス（コロンビア）
 *   90+9' ジャミントン・レアンドロ・カンパス（コロンビア）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-uzb-col.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-uzb-col.ts
...
const DEFAULT_GAME_ID = "wc-2026-K-uzb-col";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "col-munoz", teamId: "wc-col", minute: 40 },
  { playerId: "uzb-fayzullaev", teamId: "wc-uzb", minute: 60 },
  { playerId: "col-diaz", teamId: "wc-col", minute: 65 },
  { playerId: "col-campaz", teamId: "wc-col", minute: 99 },
];
