/**
 * wc-2026-A-cze-mex（チェコ 0–3 メキシコ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   55'  マテオ・チャベス（メキシコ）— mex-chavez
 *   61'  フリアン・キニョネス（メキシコ）— mex-quinones
 *   90+4' アルバロ・フィダルゴ（メキシコ）— mex-fidalgo
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-cze-mex.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-cze-mex.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-cze-mex.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-cze-mex.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-A-cze-mex";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mex-chavez", teamId: "wc-mex", minute: 55 },
  { playerId: "mex-quinones", teamId: "wc-mex", minute: 61 },
  { playerId: "mex-fidalgo", teamId: "wc-mex", minute: 94 },
];
