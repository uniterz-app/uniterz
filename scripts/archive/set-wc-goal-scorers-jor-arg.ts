/**
 * wc-2026-J-jor-arg（ヨルダン 1–3 アルゼンチン）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   19' ジオヴァニ・ロ・チェルソ（アルゼンチン）
 *   31' ラウタロ・マルティネス（アルゼンチン・PK）
 *   55' ムーサ・アル＝ターマリ（ヨルダン）
 *   80' リオネル・メッシ（アルゼンチン）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-jor-arg.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-jor-arg.ts
 *   npx tsx scripts/set-wc-goal-scorers-jor-arg.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-jor-arg.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-jor-arg.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-J-jor-arg";
