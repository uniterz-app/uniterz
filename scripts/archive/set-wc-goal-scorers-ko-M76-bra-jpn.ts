/**
 * wc-2026-ko-M76（ブラジル 2–1 日本 / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   29' 佐野海舟（日本）— jpn-sano
 *   56' カゼミーロ（ブラジル）— bra-casemiro
 *   90+6' ガブリエウ・マルティネッリ（ブラジル）— bra-martinelli
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M76-bra-jpn.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M76-bra-jpn.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M76-bra-jpn.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M76-bra-jpn.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M76";

/** ブラジル（ホーム）2 – 1 日本（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "jpn-sano", teamId: "wc-jpn", minute: 29 },
  { playerId: "bra-casemiro", teamId: "wc-bra", minute: 56 },
  { playerId: "bra-martinelli", teamId: "wc-bra", minute: 96 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    ...
    patch.regulationEtScore = { home: 2, away: 1 };
