/**
 * wc-2026-ko-M94（アメリカ 1–4 ベルギー / ラウンド16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   9'  チャールズ・デ・ケテラエル（ベルギー）— bel-ketelaere
 *   31' マリク・ティルマン（アメリカ）— usa-tillman
 *   33' チャールズ・デ・ケテラエル（ベルギー）— bel-ketelaere
 *   57' ハンス・ヴァナケン（ベルギー）— bel-vanaken
 *   90+3' ロメル・ルカク（ベルギー）— bel-lukaku
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M94-usa-bel.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M94-usa-bel.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M94-usa-bel.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M94-usa-bel.ts --with-score --resettle --force
 *   npx tsx scripts/set-wc-goal-scorers-ko-M94-usa-bel.ts --with-score --resettle --force --bracket-chain
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M94 --file=scripts/data/wc-goal-scorers-ko-M94-usa-bel.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M94";

/** アメリカ（ホーム）1 – 4 ベルギー（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "bel-ketelaere", teamId: "wc-bel", minute: 9 },
  { playerId: "usa-tillman", teamId: "wc-usa", minute: 31 },
  { playerId: "bel-ketelaere", teamId: "wc-bel", minute: 33 },
  { playerId: "bel-vanaken", teamId: "wc-bel", minute: 57 },
  { playerId: "bel-lukaku", teamId: "wc-bel", minute: 93 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 4, final: true");
  }
  ...
  const shouldBracketChain = BRACKET_CHAIN;
