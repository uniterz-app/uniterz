/**
 * wc-2026-ko-M85（スイス 2–0 アルジェリア / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   10' ブレール・エンボロ（スイス）— che-embolo
 *   46' ダン・エンドイェ（スイス）— che-ndoye
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M85-che-dza.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M85-che-dza.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M85-che-dza.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M85-che-dza.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M85";

/** スイス（ホーム）2 – 0 アルジェリア（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "che-embolo", teamId: "wc-che", minute: 10 },
  { playerId: "che-ndoye", teamId: "wc-che", minute: 46 },
];
