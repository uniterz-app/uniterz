/**
 * wc-2026-ko-M98（スペイン 2–1 ベルギー・QF）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   30' ファビアン・ルイス（スペイン）
 *   41' チャールズ・デ・ケテラエル（ベルギー）
 *   88' ミケル・メリーノ（スペイン）
 *
 *   npx tsx scripts/set-wc-goal-scorers-esp-bel.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-esp-bel.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M98";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "esp-ruiz", teamId: "wc-esp", minute: 30 },
  { playerId: "bel-ketelaere", teamId: "wc-bel", minute: 41 },
  { playerId: "esp-merino", teamId: "wc-esp", minute: 88 },
];
