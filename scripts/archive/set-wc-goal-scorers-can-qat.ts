/**
 * wc-2026-B-can-qat（カナダ 6–0 カタール）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   16' サイル・ラリン（カナダ）
 *   29', 45+3', 90+2' ジョナサン・デイヴィッド（カナダ）
 *   64' ネイサン・サリバ（カナダ）
 *   75' モハメド・マナイ OG（カタール）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-can-qat.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-can-qat.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-can-qat.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-B-can-qat";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "can-larin", teamId: "wc-can", minute: 16 },
  { playerId: "can-david", teamId: "wc-can", minute: 29 },
  { playerId: "can-david", teamId: "wc-can", minute: 48 },
  { playerId: "can-saliba", teamId: "wc-can", minute: 64 },
  { playerId: "qat-manai", teamId: "wc-qat", minute: 75, ownGoal: true },
  { playerId: "can-david", teamId: "wc-can", minute: 92 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 6 - 0, final: true");
  }
