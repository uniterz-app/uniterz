/**
 * wc-2026-ko-M99（ノルウェー 1–2 イングランド・QF / 延長）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   36' アンドレアス・シェルデルップ（ノルウェー）
 *   45+2' ジュード・ベリンガム（イングランド）
 *   93' ジュード・ベリンガム（イングランド）
 *
 *   npx tsx scripts/set-wc-goal-scorers-nor-eng.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-nor-eng.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M99";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nor-schjelderup", teamId: "wc-nor", minute: 36 },
  { playerId: "eng-bellingham", teamId: "wc-eng", minute: 47 },
  { playerId: "eng-bellingham", teamId: "wc-eng", minute: 93 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    patch.homeScore = 1;
    patch.awayScore = 2;
    patch.regulationEtScore = { home: 1, away: 2 };
    patch.advancingTeamId = "wc-eng";
    patch.final = true;
    patch.status = "final";
    patch.score = { home: 1, away: 2 };
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 2 (AET), advancing: wc-eng, final: true");
  }
