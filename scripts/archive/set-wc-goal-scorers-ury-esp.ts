/**
 * wc-2026-H-ury-esp（ウルグアイ 0–1 スペイン）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   42' アレックス・バエナ（スペイン）
 *
 *   npx tsx scripts/set-wc-goal-scorers-ury-esp.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ury-esp.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ury-esp.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-H-ury-esp";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "esp-baena", teamId: "wc-esp", minute: 42 },
];
...
(async () => {
  ...
  if (WITH_SCORE) console.log("score: 0 - 1, final: true");
