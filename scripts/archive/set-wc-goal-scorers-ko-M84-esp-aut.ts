/**
 * wc-2026-ko-M84（スペイン 3–0 オーストリア / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   36' ミケル・オヤルサバル（スペイン）— esp-oyarzabal
 *   66' ペドロ・ポロ（スペイン）— esp-porro
 *   89' ミケル・オヤルサバル（スペイン）— esp-oyarzabal
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M84-esp-aut.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M84-esp-aut.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M84-esp-aut.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M84-esp-aut.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M84";

/** スペイン（ホーム）3 – 0 オーストリア（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "esp-oyarzabal", teamId: "wc-esp", minute: 36 },
  { playerId: "esp-porro", teamId: "wc-esp", minute: 66 },
  { playerId: "esp-oyarzabal", teamId: "wc-esp", minute: 89 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-ko-M84-esp-aut.ts --with-score --resettle --force"
    );
  }
