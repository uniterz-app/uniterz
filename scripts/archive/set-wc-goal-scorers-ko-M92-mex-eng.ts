/**
 * wc-2026-ko-M92（メキシコ 2–3 イングランド / ベスト16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   36' ジュード・ベリンガム（イングランド）— eng-bellingham
 *   38' ジュード・ベリンガム（イングランド）— eng-bellingham
 *   42' フリアン・キニョネス（メキシコ）— mex-quinones
 *   60' ハリー・ケイン（イングランド / PK）— eng-kane
 *   69' ラウル・ヒメネス（メキシコ / PK）— mex-jimenez
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M92-mex-eng.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M92-mex-eng.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M92-mex-eng.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M92-mex-eng.ts --with-score --resettle --force
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M92 --file=scripts/data/wc-goal-scorers-ko-M92-mex-eng.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M92";

/** メキシコ（ホーム）2 – 3 イングランド（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "eng-bellingham", teamId: "wc-eng", minute: 36 },
  { playerId: "eng-bellingham", teamId: "wc-eng", minute: 38 },
  { playerId: "mex-quinones", teamId: "wc-mex", minute: 42 },
  { playerId: "eng-kane", teamId: "wc-eng", minute: 60 },
  { playerId: "mex-jimenez", teamId: "wc-mex", minute: 69 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 2 - 3, final: true");
  }
