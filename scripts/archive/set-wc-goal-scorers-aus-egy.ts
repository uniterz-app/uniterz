/**
 * wc-2026-ko-M88（オーストラリア 1–1 エジプト・R32 / PK 2–4 エジプト勝ち）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   13' エマム・アシュール（エジプト）
 *   55' ムハンマド・ハニー OG（オーストラリア側表示）
 *
 *   npx tsx scripts/set-wc-goal-scorers-aus-egy.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-aus-egy.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M88";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "egy-ashour", teamId: "wc-egy", minute: 13 },
  { playerId: "egy-hany", teamId: "wc-egy", minute: 55, ownGoal: true },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    ...
    patch.advancingTeamId = "wc-egy";
    patch.pkScore = { home: 2, away: 4 };
    patch.final = true;
    patch.status = "final";
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 1 (PK 2-4), advancing: wc-egy, final: true");
  }
