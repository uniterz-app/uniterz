/**
 * wc-2026-ko-M89（パラグアイ 0–1 フランス / ベスト16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   70' キリアン・エムバペ（フランス / PK）— fra-mbappe
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M89-pry-fra.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M89-pry-fra.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M89-pry-fra.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M89-pry-fra.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M89";

/** パラグアイ（ホーム）0 – 1 フランス（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 70 },
];
