/**
 * wc-2026-ko-M95（アルゼンチン 3–2 エジプト / ラウンド16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   15' ヤセル・イブラヒム（エジプト）— egy-ibrahim
 *   67' モスタファ・ジーコ（エジプト）— egy-ziko
 *   79' クリスティアン・ロメロ（アルゼンチン）— arg-romero
 *   83' リオネル・メッシ（アルゼンチン）— arg-messi
 *   90+3' エンソ・フェルナンデス（アルゼンチン）— arg-fernandez
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M95-arg-egy.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M95-arg-egy.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M95-arg-egy.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M95-arg-egy.ts --with-score --resettle --force
 *   npx tsx scripts/set-wc-goal-scorers-ko-M95-arg-egy.ts --with-score --resettle --force --bracket-chain
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M95 --file=scripts/data/wc-goal-scorers-ko-M95-arg-egy.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M95";

/** アルゼンチン（ホーム）3 – 2 エジプト（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "egy-ibrahim", teamId: "wc-egy", minute: 15 },
  { playerId: "egy-ziko", teamId: "wc-egy", minute: 67 },
  { playerId: "arg-romero", teamId: "wc-arg", minute: 79 },
  { playerId: "arg-messi", teamId: "wc-arg", minute: 83 },
  { playerId: "arg-fernandez", teamId: "wc-arg", minute: 93 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 2, final: true");
  }
