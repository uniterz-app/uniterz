/**
 * wc-2026-H-esp-sau（スペイン 4–0 サウジアラビア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   10' ラミン・ヤマル（スペイン）
 *   21' ミケル・オヤルサバル（スペイン）
 *   24' ミケル・オヤルサバル（スペイン）
 *   49' ハッサーン・トムバクティ（サウジアラビア / OG）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-esp-sau.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-esp-sau.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-esp-sau.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-esp-sau.ts --force
 *
 * 一括投入（JSON）でも同内容:
 *   npx tsx scripts/set-wc-goal-scorers.ts \
 *     --game-id=wc-2026-H-esp-sau \
 *     --file=scripts/data/wc-goal-scorers-esp-sau.json \
 *     --with-score --resettle
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（4–0 確定）
...
const DEFAULT_GAME_ID = "wc-2026-H-esp-sau";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "esp-yamal", teamId: "wc-esp", minute: 10 },
  { playerId: "esp-oyarzabal", teamId: "wc-esp", minute: 21 },
  { playerId: "esp-oyarzabal", teamId: "wc-esp", minute: 24 },
  { playerId: "sau-al-tambakti", teamId: "wc-sau", minute: 49, ownGoal: true },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 4 - 0, final: true");
  }
