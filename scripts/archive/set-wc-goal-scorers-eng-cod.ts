/**
 * wc-2026-ko-M80（イングランド 2–1 コンゴ DR・R32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   7' ブライアン・シペンガ（コンゴ DR）
 *   75' ハリー・ケイン（イングランド）
 *   86' ハリー・ケイン（イングランド）
 *
 *   npx tsx scripts/set-wc-goal-scorers-eng-cod.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-eng-cod.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M80";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "cod-cipenga", teamId: "wc-cod", minute: 7 },
  { playerId: "eng-kane", teamId: "wc-eng", minute: 75 },
  { playerId: "eng-kane", teamId: "wc-eng", minute: 86 },
];
