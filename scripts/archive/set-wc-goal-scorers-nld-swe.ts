/**
 * wc-2026-F-nld-swe（オランダ 5–1 スウェーデン）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   5'  ブライアン・ブロビー（オランダ）
 *   17' ブライアン・ブロビー（オランダ）
 *   47' コーディ・ガクポ（オランダ）
 *   54' コーディ・ガクポ（オランダ）
 *   59' アントニー・エランガ（スウェーデン）
 *   89' クリセンシオ・サマーフィル（オランダ）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-nld-swe.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-nld-swe.ts
 *   npx tsx scripts/set-wc-goal-scorers-nld-swe.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-nld-swe.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-nld-swe.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（5–1 確定）
...
const DEFAULT_GAME_ID = "wc-2026-F-nld-swe";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nld-brobbey", teamId: "wc-nld", minute: 5 },
  { playerId: "nld-brobbey", teamId: "wc-nld", minute: 17 },
  { playerId: "nld-gakpo", teamId: "wc-nld", minute: 47 },
  { playerId: "nld-gakpo", teamId: "wc-nld", minute: 54 },
  { playerId: "swe-elanga", teamId: "wc-swe", minute: 59 },
  { playerId: "nld-summerville", teamId: "wc-nld", minute: 89 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 5 - 1, final: true");
  }
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-nld-swe.ts --with-score --resettle --force"
    );
  }
