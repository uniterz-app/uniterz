/**
 * wc-2026-D-usa-aus（アメリカ 2–0 オーストラリア）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   11' キャメロン・バージェス (OG) — aus-burgess
 *   43' アレックス・フリーマン — usa-freeman
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-usa-aus.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-usa-aus.ts
 *   npx tsx scripts/set-wc-goal-scorers-usa-aus.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-usa-aus.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-usa-aus.ts --force
 *
 * 汎用 CLI:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-D-usa-aus --file=scripts/data/wc-goal-scorers-usa-aus.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-D-usa-aus";

/** アメリカ 2–0 オーストラリア（11' OG バージェス / 43' フリーマン） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "aus-burgess", teamId: "wc-aus", minute: 11, ownGoal: true },
  { playerId: "usa-freeman", teamId: "wc-usa", minute: 43 },
];
...
(async () => {
  ...
  if (DRY_RUN) {
    console.log("\n本番反映:");
    console.log(
      "  npx tsx scripts/set-wc-goal-scorers-usa-aus.ts --with-score --resettle --force"
    );
  }
