/**
 * wc-2026-ko-M77（フランス 3–0 スウェーデン / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   45' キリアン・エムバペ（フランス）— fra-mbappe
 *   53' ブラッドリー・バルコラ（フランス）— fra-barcola
 *   74' キリアン・エムバペ（フランス）— fra-mbappe
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M77-fra-swe.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M77-fra-swe.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M77-fra-swe.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M77-fra-swe.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M77";

/** フランス（ホーム）3 – 0 スウェーデン（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 45 },
  { playerId: "fra-barcola", teamId: "wc-fra", minute: 53 },
  { playerId: "fra-mbappe", teamId: "wc-fra", minute: 74 },
];
