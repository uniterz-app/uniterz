/**
 * wc-2026-ko-M100（アルゼンチン 3–1 スイス・QF / 延長）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   10' アレクシス・マック・アリスター（アルゼンチン）
 *   67' ダン・エンドイェ（スイス）
 *  112' フリアン・アルバレス（アルゼンチン）
 *  120+1' ラウタロ・マルティネス（アルゼンチン）
 *
 *   npx tsx scripts/set-wc-goal-scorers-arg-che.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-arg-che.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M100";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "arg-allister", teamId: "wc-arg", minute: 10 },
  { playerId: "che-ndoye", teamId: "wc-che", minute: 67 },
  { playerId: "arg-alvarez", teamId: "wc-arg", minute: 112 },
  { playerId: "arg-martinez-2", teamId: "wc-arg", minute: 121 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    patch.homeScore = 3;
    patch.awayScore = 1;
    patch.regulationEtScore = { home: 3, away: 1 };
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 1 (AET), advancing: wc-arg, final: true");
  }
