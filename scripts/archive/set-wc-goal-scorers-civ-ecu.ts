/**
 * wc-2026-E-civ-ecu（コートジボワール 1–0 エクアドル）の goalScorers を Firestore に投入。
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-civ-ecu.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-civ-ecu.ts
 *   npx tsx scripts/set-wc-goal-scorers-civ-ecu.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-civ-ecu.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-civ-ecu.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-E-civ-ecu";

/** コートジボワール 90' — Amad Diallo */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "civ-diallo", teamId: "wc-civ", minute: 90 },
];
