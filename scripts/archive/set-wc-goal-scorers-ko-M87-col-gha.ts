/**
 * wc-2026-ko-M87（コロンビア 1–0 ガーナ / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   14' ジョン・アリアス（コロンビア）— col-arias-2
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M87-col-gha.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M87-col-gha.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M87-col-gha.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M87-col-gha.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M87";

/** コロンビア（ホーム）1 – 0 ガーナ（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "col-arias-2", teamId: "wc-col", minute: 14 },
];
