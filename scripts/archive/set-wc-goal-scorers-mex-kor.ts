/**
 * wc-2026-A-mex-kor（メキシコ 1–0 韓国）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   50'  ルイス・ロモ（メキシコ）— mex-romo
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-mex-kor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-mex-kor.ts --with-score --force
 *   npx tsx scripts/set-wc-goal-scorers-mex-kor.ts --with-score --resettle --force
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-A-mex-kor --file=scripts/data/wc-goal-scorers-mex-kor.json --with-score
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-A-mex-kor";

/** メキシコ 50' — Luis Romo */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mex-romo", teamId: "wc-mex", minute: 50 },
];
