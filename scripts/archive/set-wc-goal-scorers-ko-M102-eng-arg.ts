/**
 * wc-2026-ko-M102（イングランド 1–2 アルゼンチン / 準決勝）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   55' アンソニー・ゴードン（イングランド）— eng-gordon
 *   85' エンソ・フェルナンデス（アルゼンチン）— arg-fernandez
 *   90+2' ラウタロ・マルティネス（アルゼンチン）— arg-martinez-2
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M102-eng-arg.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M102-eng-arg.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M102-eng-arg.ts --with-score --bracket-chain
 *   npx tsx scripts/set-wc-goal-scorers-ko-M102-eng-arg.ts --with-score --resettle --force
 *
 * 注意: 試合が既に final=true の状態で投入すると onGameFinalV2（投稿精算）が走りません。
 *       投稿が未精算のときは scripts/retrigger-on-game-final-v2.ts を実行してください。
 *
 * 汎用 JSON 投入:
 *   npx tsx scripts/set-wc-goal-scorers.ts --game-id=wc-2026-ko-M102 --file=scripts/data/wc-goal-scorers-ko-M102-eng-arg.json --with-score
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M102";

/** イングランド（ホーム）1 – 2 アルゼンチン（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "eng-gordon", teamId: "wc-eng", minute: 55 },
  { playerId: "arg-fernandez", teamId: "wc-arg", minute: 85 },
  { playerId: "arg-martinez-2", teamId: "wc-arg", minute: 92 },
];
...
(async () => {
  ...
  /** Firestore 上の配置（検証済み）: home=eng / away=arg */
  const EXPECTED_HOME = "wc-eng";
  const EXPECTED_AWAY = "wc-arg";
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 2, final: true");
  }
