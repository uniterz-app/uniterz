/**
 * wc-2026-G-nzl-egy（ニュージーランド 1–3 エジプト）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   15' フィン・サーマン（ニュージーランド）
 *   58' モスタファ・ズィーコ（エジプト）
 *   67' モハメド・サラー（エジプト）
 *   82' マフムード・ハッサン（エジプト / 名簿: Haissem Hassan）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-nzl-egy.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-nzl-egy.ts
 *   npx tsx scripts/set-wc-goal-scorers-nzl-egy.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-nzl-egy.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-nzl-egy.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-G-nzl-egy";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nzl-surman", teamId: "wc-nzl", minute: 15 },
  { playerId: "egy-ziko", teamId: "wc-egy", minute: 58 },
  { playerId: "egy-salah", teamId: "wc-egy", minute: 67 },
  { playerId: "egy-hassan", teamId: "wc-egy", minute: 82 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 1 - 3, final: true");
  }
