/**
 * wc-2026-E-cuw-civ（キュラソー 0–2 コートジボワール）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   7'  ニコラ・ペペ（コートジボワール）
 *   64' ニコラ・ペペ（コートジボワール）
 ...
 *
 *   npx tsx scripts/set-wc-goal-scorers-cuw-civ.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-cuw-civ.ts
 *   npx tsx scripts/set-wc-goal-scorers-cuw-civ.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-cuw-civ.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-cuw-civ.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（0–2 確定）
...
const DEFAULT_GAME_ID = "wc-2026-E-cuw-civ";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "civ-pepe", teamId: "wc-civ", minute: 7 },
  { playerId: "civ-pepe", teamId: "wc-civ", minute: 64 },
];
