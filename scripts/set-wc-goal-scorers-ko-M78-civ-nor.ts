/**
 * wc-2026-ko-M78（コートジボワール 1–2 ノルウェー / ベスト32）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   39' アントニオ・ヌサ（ノルウェー）— nor-nusa
 *   74' アマド・ディアロ（コートジボワール）— civ-diallo
 *   86' アーリング・ブラウト・ハーランド（ノルウェー）— nor-haaland
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M78-civ-nor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M78-civ-nor.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M78-civ-nor.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M78-civ-nor.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M78";

/** コートジボワール（ホーム）1 – 2 ノルウェー（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nor-nusa", teamId: "wc-nor", minute: 39 },
  { playerId: "civ-diallo", teamId: "wc-civ", minute: 74 },
  { playerId: "nor-haaland", teamId: "wc-nor", minute: 86 },
];
