/**
 * wc-2026-ko-M86（アルゼンチン 3–2 カーボベルデ・R32 / 延長）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   29'  リオネル・メッシ（アルゼンチン）
 *   59'  デロイ・ドゥアルテ（カーボベルデ）
 *   92'  リサンドロ・マルティネス（アルゼンチン）
 *  103'  シドニー・カブラル（カーボベルデ）
 *  111'  ディニー OG（アルゼンチン側表示）
 *
 *   npx tsx scripts/set-wc-goal-scorers-arg-cpv.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-arg-cpv.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M86";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "arg-messi", teamId: "wc-arg", minute: 29 },
  { playerId: "cpv-duarte", teamId: "wc-cpv", minute: 59 },
  { playerId: "arg-martinez", teamId: "wc-arg", minute: 92 },
  { playerId: "cpv-cabral-2", teamId: "wc-cpv", minute: 103 },
  { playerId: "cpv-diney", teamId: "wc-cpv", minute: 111, ownGoal: true },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    patch.homeScore = 3;
    patch.awayScore = 2;
    patch.regulationEtScore = { home: 3, away: 2 };
    patch.advancingTeamId = "wc-arg";
    patch.final = true;
    patch.status = "final";
    patch.score = { home: 3, away: 2 };
    patch.finalMeta = { ot: true };
  ...
  if (WITH_SCORE) {
    console.log("score: 3 - 2 (AET), advancing: wc-arg, final: true");
  }
