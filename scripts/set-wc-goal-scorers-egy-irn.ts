/**
 * wc-2026-G-egy-irn（エジプト 1–1 イラン）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   5'  マフムード・サベル（エジプト）
 *   14' ラミン・レザイーアン（イラン）
 *
 *   npx tsx scripts/set-wc-goal-scorers-egy-irn.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-egy-irn.ts --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-G-egy-irn";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "egy-saber", teamId: "wc-egy", minute: 5 },
  { playerId: "irn-rezaeian", teamId: "wc-irn", minute: 14 },
];
...
(async () => {
  ...
  if (WITH_SCORE) console.log("score: 1 - 1, final: true");
