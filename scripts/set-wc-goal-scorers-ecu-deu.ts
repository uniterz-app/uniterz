/**
 * wc-2026-E-ecu-deu（エクアドル 2–1 ドイツ）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   2'  レロイ・サネ（ドイツ）
 *   9'  ニルソン・アングロ（エクアドル）
 ...
 *
 *   npx tsx scripts/set-wc-goal-scorers-ecu-deu.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ecu-deu.ts
 *   npx tsx scripts/set-wc-goal-scorers-ecu-deu.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ecu-deu.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ecu-deu.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
...
const DEFAULT_GAME_ID = "wc-2026-E-ecu-deu";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "deu-sane", teamId: "wc-deu", minute: 2 },
  { playerId: "ecu-angulo", teamId: "wc-ecu", minute: 9 },
  { playerId: "ecu-plata", teamId: "wc-ecu", minute: 77 },
];
