/**
 * wc-2026-ko-M90（カナダ 0–3 モロッコ / ベスト16）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   50' アゼディン・ウナヒ（モロッコ）— mar-ounahi
 *   82' アゼディン・ウナヒ（モロッコ）— mar-ounahi
 *   90+8' ソフィアン・ラヒミ（モロッコ）— mar-rahimi
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-ko-M90-can-mar.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-ko-M90-can-mar.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-ko-M90-can-mar.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-ko-M90-can-mar.ts --with-score --resettle --force
 */
...
const DEFAULT_GAME_ID = "wc-2026-ko-M90";

/** カナダ（ホーム）0 – 3 モロッコ（アウェイ） */
const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "mar-ounahi", teamId: "wc-mar", minute: 50 },
  { playerId: "mar-ounahi", teamId: "wc-mar", minute: 82 },
  { playerId: "mar-rahimi", teamId: "wc-mar", minute: 98 },
];
...
(async () => {
  ...
  if (WITH_SCORE) {
    console.log("score: 0 - 3, final: true");
  }
