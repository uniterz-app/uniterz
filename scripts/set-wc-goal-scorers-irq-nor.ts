/**
 * wc-2026-I-irq-nor（イラク 1–4 ノルウェー）の goalScorers を Firestore に投入。
 *
 * 得点:
 *   29' アーリング・ブラウト・ハーランド（ノルウェー）
 *   39' アイマン・フサイン（イラク）
 *   43' アーリング・ブラウト・ハーランド（ノルウェー）
 *   76' レオ・エスティゴーア（ノルウェー）
 *   90+7' クリスチャン・トルストベット（ノルウェー）
 *
 * 前提: プロジェクトルートに service-account.json または serviceAccount.json
 *
 *   npx tsx scripts/set-wc-goal-scorers-irq-nor.ts --dry-run
 *   npx tsx scripts/set-wc-goal-scorers-irq-nor.ts
 *   npx tsx scripts/set-wc-goal-scorers-irq-nor.ts --with-score
 *   npx tsx scripts/set-wc-goal-scorers-irq-nor.ts --with-score --resettle
 *   npx tsx scripts/set-wc-goal-scorers-irq-nor.ts --force
 *
 * オプション:
 *   --dry-run     書き込まず内容を表示
 *   --with-score  homeScore/awayScore/final/status も更新（1–4 確定）
...
const DEFAULT_GAME_ID = "wc-2026-I-irq-nor";

const GOAL_SCORERS: WcGameGoalScorer[] = [
  { playerId: "nor-haaland", teamId: "wc-nor", minute: 29 },
  { playerId: "irq-hussein", teamId: "wc-irq", minute: 39 },
  { playerId: "nor-haaland", teamId: "wc-nor", minute: 43 },
  { playerId: "nor-ostigard", teamId: "wc-nor", minute: 76 },
  { playerId: "nor-thorstvedt", teamId: "wc-nor", minute: 97 },
];
