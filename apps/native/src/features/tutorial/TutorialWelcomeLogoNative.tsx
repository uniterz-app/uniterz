...
function WelcomeLetterNative({
  ...
  const x = useSharedValue(skipIntro ? 0 : fromX);
  const y = useSharedValue(skipIntro ? 0 : fromY);
  const rot = useSharedValue(skipIntro ? 0 : letter.scatter.rotate);
  const scale = useSharedValue(skipIntro ? 1 : 0.86);
  ...
  useEffect(() => {
    ...
    // 画面サイズ変化で集合をやり直さない（入場は1回）
  }, [index, letter.scatter.rotate, op, flash, reduceMotion, rot, scale, skipIntro, x, y]);
...
export function WelcomeGatherNative({
  ...
  const x = useSharedValue(skipIntro ? 0 : fromX);
  const y = useSharedValue(skipIntro ? 0 : fromY);
  const op = useSharedValue(skipIntro ? 1 : 0);
  ...
  }, [delayMs, fromX, fromY, op, reduceMotion, skipIntro, x, y]);
