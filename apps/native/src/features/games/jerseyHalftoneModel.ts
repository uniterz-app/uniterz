...
export function buildJerseyHalftoneDotList(
  ...
  const resolvedSingleDotColor = gradientMode
    ? null
    : blendAccentForJerseyDots(accent);

  const gradientRgb =
    gradientMode && accentEnd
      ? {
          start: parseJerseyHexToRgb(accent),
          end: parseJerseyHexToRgb(accentEnd),
        }
      : null;
  ...
  for (let gy = oy + step * 0.5; gy < oy + VIEWBOX_H * s; gy += step) {
    for (let gx = ox + step * 0.5; gx < ox + VIEWBOX_W * s; gx += step) {
      ...
      if (gradientRgb) {
        ...
        const c = liftRgbForJerseyDisplay(
          lerpRgb(gradientRgb.start, gradientRgb.end, t),
          flat ? 0 : JERSEY_BODY_LIFT * 0.9
        );
        fill = jerseyRgbToCss(c);
      } else {
        fill = flat
          ? jerseyRgbToCss(parseJerseyHexToRgb(accent))
          : (resolvedSingleDotColor ?? blendAccentForJerseyDots(accent));
      }
