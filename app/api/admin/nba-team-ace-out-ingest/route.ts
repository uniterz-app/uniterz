...
/**
 * POST /api/admin/nba-team-ace-out-ingest
 * チームエース欠場時 W–L → Firestore `nbaTeamAceOutRecords/{seasonKey}`。
 * 認証: Admin UID または job secret。
 *
 * body: { seasonKey?: "2025-26", force?: boolean, minGp?: number }
 */
export async function POST(req: Request) {
  try {
    ...
    const result = await ingestNbaTeamAceOutRecords(getAdminDb(), {
      ...
      minGp: typeof body.minGp === "number" ? body.minGp : undefined,
    });
    ...
    console.error("[nba-team-ace-out-ingest]", e);
