...
export default function MobileBookmarksPage() {
  ...
  createdAtMillis: createdAt ? createdAt.getTime() : undefined,

  // ---- Game Required Fields (V2仕様) ----
  league: d.league ?? d.game?.league ?? "bj",
  status: d.status ?? d.game?.status ?? "scheduled",
  gameId: d.gameId ?? d.game?.gameId ?? null,

  // V2の home / away
  home: {
    name: d.home?.name ?? d.game?.home ?? "",
    teamId: d.home?.teamId ?? "",
    number: d.home?.number,
    record: d.home?.record ?? null,
  },
  away: {
    name: d.away?.name ?? d.game?.away ?? "",
    teamId: d.away?.teamId ?? "",
    number: d.away?.number,
    record: d.away?.record ?? null,
  },

  // ----- 旧gameブロック（残すだけでUIでは使わない） -----
  game: {
    league: d.league ?? d.game?.league ?? "bj",
    home:
      typeof g.home === "string" ? g.home : g.home?.name ?? d.home?.name ?? "",
    away:
      typeof g.away === "string" ? g.away : g.away?.name ?? d.away?.name ?? "",
  ...
  return (
    <main className="min-h-screen bg-black text-white px-4 py-4">
      ...
      {needLogin ? (
        ...
        <p className="text-sm text-white/70">まだブックマークがありません。</p>
      ) : (
        <div className="space-y-4 pb-8">
          {posts.map((p) => {
            const handle =
              (p.authorHandle ?? "").replace(/^@/, "") || p.authorUid || "";
