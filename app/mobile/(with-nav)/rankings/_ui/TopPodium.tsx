...
function metricNum(r: RankingRowWithCountry, metric: MobileMetric) {
  if (metric === "totalScore") {
    return { n: r.totalScore ?? 0, d: 0 };
  }
  if (metric === "marginPrecision") {
    return { n: r.marginPrecisionScore ?? 0, d: 0 };
  }
  if (metric === "upsetScore") {
    return { n: r.upsetScore ?? 0, d: 0 };
  }
  if (metric === "winRate") {
...
export default function TopPodium({
  ...
  const v1n = useCountUp(a1.n, 900, a1.d, true, onTopCountDone);
  const v2n = useCountUp(a2?.n ?? 0, 900, a2?.d ?? 0, !!r2);
  const v3n = useCountUp(a3?.n ?? 0, 900, a3?.d ?? 0, !!r3);
