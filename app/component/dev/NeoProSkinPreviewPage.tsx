...
/**
 * NEO LAB Pro Skin 案プレビュー（10 案）。
 * 既存の線画タイル系とは別系統の実験的スキン。
 * 本番採用リストには未追加 — 比較・選定用。
 */
...
function ScaledCard({
  ...
  scale: number;
  skinId: ProfilePlanProNeoBgVariant;
}) {
...
export default function NeoProSkinPreviewPage({ variant }: Props) {
  const entries = PROFILE_PLAN_PRO_NEO_BG_VARIANTS;
  const [selectedId, setSelectedId] = useState<ProfilePlanProNeoBgVariant>(
    entries[0]!.id
  );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <CyberSubpageShell
        ...
        eyebrow="PRO SKIN"
        title="NEO LAB"
        subtitle="既存タイル系とは別系統の実験的 10 案。タイルで切替。本番採用リストには未追加。"
        onBack={() => {
      ...
      >
        <p
          className={[
            ...
            "mb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-fuchsia-300/70",
          ].join(" ")}
        >
          Preview only · Neo Round 1
        </p>
        <p
          className={[
            jp.className,
        ...
        >
          色の霧ではなく、線・格子・波・亀裂など「柄」として読める 10
          案。下のタイルで比較できます。
        </p>
        ...
        <div className="mb-3">
          <p
            className={[
              nameOxanium.className,
              "text-[10px] font-bold uppercase tracking-[0.16em] text-fuchsia-200/70",
            ].join(" ")}
          >
            {selected.tag}
          </p>
