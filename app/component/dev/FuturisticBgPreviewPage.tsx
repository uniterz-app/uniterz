/**
 * Futuristic BG — 実プロフィールカードに当てはめて比較（選定用・本番未接続）。
 */
"use client";
...
import { FUTURISTIC_BG_THEME as T } from "@/lib/profile/futuristicBgTheme";
...
function ScaledCard({
  ...
  scale: number;
  skinId: ProfilePlanProFuturisticBgVariant;
}) {
...
export default function FuturisticBgPreviewPage({ variant }: Props) {
  const entries = PROFILE_PLAN_PRO_FUTURISTIC_BG_VARIANTS;
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <CyberSubpageShell
        ...
        eyebrow="PRO SKIN"
        title="FUTURISTIC"
        subtitle="採用スキン Eclipse / Data Stream を実カードに当てはめて確認。"
        onBack={() => {
      ...
      >
        <p
          ...
          style={{ color: T.cyanAlpha.strong }}
        >
          Preview only · Futuristic Round 1
        </p>
        <p
          className={[jp.className, "mb-5 text-xs leading-relaxed text-white/45"].join(
            " ",
          )}
        >
          下のタイルで切替。カード上の文字・枠・メトリクスと一緒に見え方を確認できます。
        </p>
        ...
        <div className="mb-3">
          ...
          <p
            className={[jp.className, "mt-1 text-xs leading-relaxed text-white/50"].join(
              " ",
            )}
          >
