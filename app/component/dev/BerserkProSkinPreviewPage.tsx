...
function ScaledCard({
  ...
  scale: number;
  skinId: ProfilePlanProBeastBgVariant;
}) {
...
export default function BerserkProSkinPreviewPage({ variant }: Props) {
  const entries = useMemo(
    () =>
      PROFILE_PLAN_PRO_BEAST_BG_ROUND6.map(
        (id) => PROFILE_PLAN_PRO_BEAST_BG_VARIANTS.find((v) => v.id === id)!
      ),
    []
  );
  const [selectedId, setSelectedId] = useState<ProfilePlanProBeastBgVariant>(
    PROFILE_PLAN_PRO_BEAST_BG_ROUND6[0]!
  );
      ...
      >
        <p
          className={[
            ...
            "mb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-lime-300/70",
          ].join(" ")}
        >
          Preview only · Round 6
        </p>
        <p className={[jp.className, "mb-5 text-xs leading-relaxed text-white/45"].join(" ")}>
          ライムグリーンの粒子二重らせん「Helix Genome」を追加。下のタイルで比較できます。
        </p>
