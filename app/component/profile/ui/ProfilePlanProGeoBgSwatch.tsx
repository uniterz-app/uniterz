...
import type { ProfilePlanProGeoBgMeta } from "@/lib/profile/profilePlanProGeoBgVariants";

type Props = {
  variant: ProfilePlanProGeoBgMeta;
  selected?: boolean;
  onSelect?: () => void;
};

/** 幾何学背景 — コンパクトプレビュー */
export default function ProfilePlanProGeoBgSwatch({
  variant,
  selected = false,
  onSelect,
}: Props) {
  ...
  return (
    <Tag
      ...
      onClick={onSelect}
      className={[
        "profile-plan-pro-geo-swatch group relative w-full overflow-hidden rounded-xl border text-left transition",
    ...
    >
      ...
      <div className="relative z-[1] flex min-h-[120px] flex-col justify-end p-3 sm:min-h-[140px]">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="rounded border border-emerald-400/35 bg-emerald-400/10 px-1.5 py-px text-[9px] text-emerald-100/90">
            GEO
          </span>
        </div>
