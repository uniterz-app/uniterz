...
import type { ProfilePlanProHexBgMeta } from "@/lib/profile/profilePlanProHexBgVariants";

type Props = {
  variant: ProfilePlanProHexBgMeta;
  selected?: boolean;
  onSelect?: () => void;
};

/** 六角レイアウト — コンパクトプレビュー */
export default function ProfilePlanProHexBgSwatch({
  variant,
  selected = false,
  onSelect,
}: Props) {
  ...
  return (
    <Tag
      ...
      onClick={onSelect}
      className={[
        "profile-plan-pro-hex-swatch group relative w-full overflow-hidden rounded-xl border text-left transition",
    ...
    >
      ...
      <div className="relative z-[1] flex min-h-[120px] flex-col justify-end p-3 sm:min-h-[140px]">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="rounded border border-cyan-400/35 bg-cyan-400/10 px-1.5 py-px text-[9px] text-cyan-100/90">
            {variant.tag}
          </span>
        </div>
