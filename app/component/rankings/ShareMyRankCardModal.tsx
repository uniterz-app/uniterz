...
type Props = {
  open: boolean;
  payload: RankCardImagePayload | null;
  language: Language;
  onClose: () => void;
};

export default function ShareMyRankCardModal({
  open,
  payload,
  language,
  onClose,
  ...
  const urls = useMemo(
    () => buildRankCardShareUrls(language, payload?.shareText),
    [language, payload?.shareText]
  );
  ...
  const onShareToApps = useCallback(async () => {
    ...
    if (!ok && payload) {
      downloadRankCardImage(payload.dataUrl);
  ...
  const onShareLine = useCallback(async () => {
    ...
    if (payload) downloadRankCardImage(payload.dataUrl);
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal
      aria-labelledby="share-rank-card-title"
    >
