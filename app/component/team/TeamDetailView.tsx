...
import { useStandings } from "@/app/component/standings/useStandings";
...
const list = conference === "east" ? east : west;

// teamDetail は teams/[teamId]/page.tsx で id を渡してるので team.id を使う
const idx = list.findIndex((t) => t.id === team.id);
const conferenceRank = idx >= 0 ? idx + 1 : null;
...
<motion.div
  ...
  style={{
    background: `
      ...
      radial-gradient(700px 380px at 95% 15%, rgba(140,80,255,0.20), transparent 55%),
      radial-gradient(600px 300px at 50% 120%, rgba(0,200,180,0.18), transparent 60%),
      linear-gradient(180deg, #0a0e16 0%, #05070c 55%, #02040a 100%)
    `,
  }}
>
    ...
    <div
      ...
      style={{
        background: `
          radial-gradient(900px 420px at 15% -10%, rgba(0,220,255,0.14), transparent 60%),
          radial-gradient(700px 380px at 95% 15%, rgba(140,80,255,0.12), transparent 55%),
          radial-gradient(600px 300px at 50% 120%, rgba(0,200,180,0.10), transparent 60%),
          linear-gradient(180deg, rgba(255,255,255,0.03), rgba(0,0,0,0.0) 40%, rgba(0,0,0,0.35))
        `,
      }}
    />
    ...
    <div
      ...
      style={{
        backgroundImage:
          "url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"120\" height=\"120\"><filter id=\"n\"><feTurbulence type=\"fractalNoise\" baseFrequency=\"0.9\" numOctaves=\"1\" stitchTiles=\"stitch\"/></filter><rect width=\"120\" height=\"120\" filter=\"url(%23n)\" opacity=\"0.035\"/></svg>')",
...
<div className={`${alfa.className} text-4xl font-bold tabular-nums`}>
  ...
  {mode === "away" && away && `${away.wins}-${away.losses}`}
            ...
            <div className="absolute left-0 bottom-0 px-4 py-2 text-white bg-transparent">
  <button
    onClick={() => setMode(mode === "total" ? "home" : mode === "home" ? "away" : "total")}
...
<DepthCard accent={confMode === "east" ? "#EF3B24" : "#007AC1"} classNameOverride="p-4">
  ...
  >
    ...
    <div className="flex items-end justify-between gap-2">
<div
  className={`${alfa.className} text-[30px] leading-none font-bold tabular-nums whitespace-nowrap shrink-0`}
  style={{
