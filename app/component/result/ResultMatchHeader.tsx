...
function fmtRecordWithRank(r: { wins: number; losses: number; rank?: number } | null) {
  if (!r) return "(0-0)";
...
function useTeamRecord(teamId?: string) {
  const [rec, setRec] = useState<{ wins: number; losses: number; rank?: number } | null>(null);
...
/** Mobile表示用チーム名 */
function getMobileTeamName(league: string, rawName: string, l1: string, l2?: string) {
...
export default function ResultMatchHeader({ post }: Props) {
  ...
  const homeColor = getTeamPrimaryColor(normalizedLeague, post.home?.teamId) ?? "#0ea5e9";
  const awayColor = getTeamPrimaryColor(normalizedLeague, post.away?.teamId) ?? "#f43f5e";
  ...
  const hasFinal = typeof post.result?.home === "number" && typeof post.result?.away === "number";
  ...
  // ★ MatchCardと同じデータ源（teams/{teamId}）から成績/順位
  ...
  let frame = "border border-cyan-400/30 shadow-[0_0_40px_rgba(56,189,248,0.15)]";
  if (badge === "upset") {
    frame =
  ...
  // ★ 色合わせ（Pro系カードと同じ）
  // rounded-2xl border border-white/15 bg-[#050814]/80 ... のトーンに寄せる
  const cardBase =
    "rounded-2xl border border-white/15 bg-[#050814]/80 p-10 text-white overflow-hidden shadow-[0_14px_40px_rgba(0,0,0,0.55)]";

  // ★ 修正：自信度は post.prediction.confidence を参照（Mobile と同じ）
  // 想定：0..100（%） or 0..1（比率）どちらでも表示できるように吸収
  const confPred =
  ...
  return (
    <div className={`relative ${cardBase} ${frame}`}>
      ...
      <div className="absolute left-4 top-4 z-10">
        <span
          className="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-[12px] font-extrabold tracking-wide"
      ...
      {/* 右上：結果バッジ */}
      {badge === "hit" && (
        <span className="absolute right-4 top-4 z-10 bg-yellow-400 text-black text-[12px] px-2.5 py-1 rounded-md font-extrabold shadow-md">
          HIT
        </span>
      )}
      {badge === "upset" && (
        <span className="absolute right-4 top-4 z-10 bg-red-500 text-white text-[12px] px-2.5 py-1 rounded-md font-extrabold shadow-md">
          UPSET
        </span>
      )}
      {badge === "miss" && (
        <span className="absolute right-4 top-4 z-10 bg-gray-500 text-white text-[12px] px-2.5 py-1 rounded-md font-extrabold shadow-md">
          MISS
        </span>
      )}
      ...
      <div className="grid grid-cols-3 items-center">
        ...
        <div className="flex flex-col items-center">
          <Icon className="w-16 h-16" fill={homeColor} stroke="#fff" />
          <div className="mt-2 text-[20px] text-center leading-tight font-bold">
            {getMobileTeamName(post.league, post.home?.name ?? "", homeL1, homeL2)}
          </div>
          <div className="mt-1 text-[16px] opacity-70 leading-none tracking-tight text-center">
        ...
        <div className="flex flex-col items-center justify-center">
          {matchDate && <div className="text-[18px] opacity-80 mb-2">{matchDate}</div>}
          ...
          {/* ★ 自信度（予想スコアと Final の間） */}
          {confPct !== null && (
            <div className="mt-3 text-[14px] font-semibold tabular-nums text-white/70">
              自信度: {confPct}%
            </div>
          )}
        ...
        {/* AWAY */}
        <div className="flex flex-col items-center">
          <Icon className="w-16 h-16" fill={awayColor} stroke="#fff" />
          <div className="mt-2 text-[20px] text-center leading-tight font-bold">
