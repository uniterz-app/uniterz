...
| **INJURY IMPACT** | 欠場でチームの形・欠場時成績がどう変わるか（队友バンプ予測はしない · 正: `docs/pro-insight-generation-rules.md`） |
...
> ※ 下の「役割が増加」例は旧案。現行正は欠場時 W–L / OFF·DEF と柱（得点リーダー等）が落ちる読みのみ。队友バンプ予測は出さない（`docs/pro-insight-generation-rules.md`）。
...
| 進行中 | **廃止**（live 日次更新なし） |
| 確定 | 翌月曜 08:30 JST に確定（`status: final`）のみ |
| プッシュ | **確定時のみ** |
| トライアル | 開始後、最初の月曜 final 配信まで `proUntil` を底上げ。その週に1予想以上あれば必ず1通 |
| 対象 | NBA / Pro のみ |
