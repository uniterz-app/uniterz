...
exports.getThisMonthRangeJst = getThisMonthRangeJst;
exports.getLastMonthRangeJst = getLastMonthRangeJst;
exports.getThisWeekRangeJst = getThisWeekRangeJst;
exports.getLastWeekRangeJst = getLastWeekRangeJst;
...
function jstYmdToUtcDate(y, m, d) {
 ...
 * =======================*/
/** 今月のレンジ（JST 基準） */
function getThisMonthRangeJst(today = new Date()) {
...
/** 先月のレンジ（JST 基準） */
function getLastMonthRangeJst(today = new Date()) {
    const j = toJstDate(today);
...
function getWeekMondayJstDate(j) {
    // 0=Sun,1=Mon,...6=Sat
    const dow = j.getUTCDay();
    const offsetToMonday = dow === 0 ? -6 : 1 - dow; // 日曜なら -6, 月曜なら 0, ...
    const mondayJstUtc = Date.UTC(j.getUTCFullYear(), j.getUTCMonth(), j.getUTCDate() + offsetToMonday, 0, 0, 0);
    return new Date(mondayJstUtc);
}
/** 今週（月曜〜来週月曜）のレンジ（JST 基準） */
function getThisWeekRangeJst(today = new Date()) {
    const j = toJstDate(today);
...
function getLastWeekRangeJst(today = new Date()) {
