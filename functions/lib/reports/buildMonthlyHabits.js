"use strict";
// synced from lib/reports/buildMonthlyHabits.ts — run npm run sync:monthly-report-builders
// 月次レポート「予想のクセ」— 集計値 → スタイルマップ用バイアス + 勝率 + 短文。
// docs/pro-subscription-plan.md §4 予想のクセ（Pro Stats ハイブリッド）
Object.defineProperty(exports, "__esModule", { value: true });
exports.MONTHLY_HABIT_MIN_SIDE_POSTS = void 0;
exports.computeHomeAwayBias = computeHomeAwayBias;
exports.computeMarketBias = computeMarketBias;
exports.buildHabitsStyleComment = buildHabitsStyleComment;
exports.buildMonthlyHabits = buildMonthlyHabits;
exports.MONTHLY_HABIT_MIN_SIDE_POSTS = 3;
...
function computeHomeAwayBias(home, away) {
...
function computeMarketBias(favorite, underdog) {
...
function buildHabitsStyleComment(homeAwayBias, marketBias, winRate, lang = "ja") {
...
function sidesReady(raw) {
    return (raw.home.posts >= exports.MONTHLY_HABIT_MIN_SIDE_POSTS &&
        raw.away.posts >= exports.MONTHLY_HABIT_MIN_SIDE_POSTS &&
        raw.favorite.posts >= exports.MONTHLY_HABIT_MIN_SIDE_POSTS &&
        raw.underdog.posts >= exports.MONTHLY_HABIT_MIN_SIDE_POSTS);
}
/**
 * 集計済み Home/Away・市場 → クセブロック。
 * 各サイド最低投稿に満たない場合は null。
 */
function buildMonthlyHabits(raw, lang = "ja") {
    var _a;
    if (!sidesReady(raw))
        return null;
    ...
    const allPosts = raw.home.posts + raw.away.posts + raw.favorite.posts + raw.underdog.posts;
    ...
    const winRate = (_a = raw.winRate) !== null && _a !== void 0 ? _a : (haPosts > 0 ? haWins / haPosts : allPosts > 0 ? 0 : 0);
    const comment = buildHabitsStyleComment(homeAwayBias, marketBias, winRate, lang);
    return {
        homeAwayBias,
        marketBias,
        winRate,
...
//# sourceMappingURL=buildMonthlyHabits.js.map